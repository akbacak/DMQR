function varargout = dmqRetrieval_5d(varargin)
% DMQRETRIEVAL_5D MATLAB code for dmqRetrieval_5d.fig
%      DMQRETRIEVAL_5D, by itself, creates a new DMQRETRIEVAL_5D or raises the existing
%      singleton*.
%
%      H = DMQRETRIEVAL_5D returns the handle to a new DMQRETRIEVAL_5D or the handle to
%      the existing singleton*.
%
%      DMQRETRIEVAL_5D('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in DMQRETRIEVAL_5D.M with the given input arguments.
%
%      DMQRETRIEVAL_5D('Property','Value',...) creates a new DMQRETRIEVAL_5D or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before dmqRetrieval_5d_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to dmqRetrieval_5d_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help dmqRetrieval_5d

% Last Modified by GUIDE v2.5 20-Apr-2020 13:24:48

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @dmqRetrieval_5d_OpeningFcn, ...
                   'gui_OutputFcn',  @dmqRetrieval_5d_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before dmqRetrieval_5d is made visible.
function dmqRetrieval_5d_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to dmqRetrieval_5d (see VARARGIN)

% Choose default command line output for dmqRetrieval_5d
handles.output = hObject;

% Update handles structure
guidata(hObject, handles);

% UIWAIT makes dmqRetrieval_5d wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = dmqRetrieval_5d_OutputFcn(hObject, eventdata, handles) 
...
% handles    structure with handles and user data (see GUIDATA)
    ...
    queryIndex4 = handles.q4Idx;
    queryIndex5 = handles.q5Idx;
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    q4_label = targets(queryIndex4,: );
    q5_label = targets(queryIndex5,: );
...
[frontSize, e] = size(pf_idx{currentFront,1}(:,6));
...
set(handles.QueryName2,'String', filenames);
set(handles.QueryName3,'String', filenames);
set(handles.QueryName4,'String', filenames);
set(handles.QueryName5,'String', filenames);
...
% --- Executes on selection change in QueryName5.
function QueryName5_Callback(hObject, eventdata, handles)
% hObject    handle to QueryName5 (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hints: contents = cellstr(get(hObject,'String')) returns QueryName5 contents as cell array
%        contents{get(hObject,'Value')} returns selected item from QueryName5
imindex = get(hObject,'Value');
image_dir = handles.image_dir;
fname = [image_dir handles.filenames{imindex}];
axes(handles.axes14);
imshow(imread(fname)); axis image;
handles.q5Idx = imindex;
guidata(hObject, handles);

% --- Executes during object creation, after setting all properties.
function QueryName5_CreateFcn(hObject, eventdata, handles)
% hObject    handle to QueryName5 (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    empty - handles not created until after all CreateFcns called
