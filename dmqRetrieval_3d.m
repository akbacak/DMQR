function varargout = dmqRetrieval_3d(varargin)
% DMQRETRIEVAL_3D MATLAB code for dmqRetrieval_3d.fig
%      DMQRETRIEVAL_3D, by itself, creates a new DMQRETRIEVAL_3D or raises the existing
%      singleton*.
%
%      H = DMQRETRIEVAL_3D returns the handle to a new DMQRETRIEVAL_3D or the handle to
%      the existing singleton*.
%
%      DMQRETRIEVAL_3D('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in DMQRETRIEVAL_3D.M with the given input arguments.
%
%      DMQRETRIEVAL_3D('Property','Value',...) creates a new DMQRETRIEVAL_3D or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before dmqRetrieval_3d_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to dmqRetrieval_3d_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help dmqRetrieval_3d

% Last Modified by GUIDE v2.5 04-Dec-2018 14:07:42

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @dmqRetrieval_3d_OpeningFcn, ...
                   'gui_OutputFcn',  @dmqRetrieval_3d_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before dmqRetrieval_3d is made visible.
function dmqRetrieval_3d_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to dmqRetrieval_3d (see VARARGIN)

% Choose default command line output for dmqRetrieval_3d
handles.output = hObject;

% Update handles structure
guidata(hObject, handles);

% UIWAIT makes dmqRetrieval_3d wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = dmqRetrieval_3d_OutputFcn(hObject, eventdata, handles) 
