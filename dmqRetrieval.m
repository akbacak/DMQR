function varargout = dmqRetrieval(varargin)
% DMQRETRIEVAL MATLAB code for dmqRetrieval.fig
%      DMQRETRIEVAL, by itself, creates a new DMQRETRIEVAL or raises the existing
%      singleton*.
%
%      H = DMQRETRIEVAL returns the handle to a new DMQRETRIEVAL or the handle to
%      the existing singleton*.
%
%      DMQRETRIEVAL('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in DMQRETRIEVAL.M with the given input arguments.
%
%      DMQRETRIEVAL('Property','Value',...) creates a new DMQRETRIEVAL or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before dmqRetrieval_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to dmqRetrieval_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help dmqRetrieval

% Last Modified by GUIDE v2.5 04-Dec-2018 13:40:16

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @dmqRetrieval_OpeningFcn, ...
                   'gui_OutputFcn',  @dmqRetrieval_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before dmqRetrieval is made visible.
function dmqRetrieval_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to dmqRetrieval (see VARARGIN)

% Choose default command line output for dmqRetrieval
handles.output = hObject;

% Update handles structure
guidata(hObject, handles);

% UIWAIT makes dmqRetrieval wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = dmqRetrieval_OutputFcn(hObject, eventdata, handles) 
